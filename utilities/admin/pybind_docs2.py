...
def extract_docs_from_xml():
    ...
    DEBUG_JSON.write_text(json.dumps(dump, indent=2), encoding="utf-8")
...
def insert_docs_into_bindings():
    """
    Robust insertion: parse each .def(...) call, split top-level args, and insert docstring
        ...
        while True:
            ...
            if not has_docstring:
                ...
                if func_entry:
                    ...
                    if docstring:
                        escaped = escape_docstring_for_cpp(docstring)
                        # Where to insert? Insert after the second argument.
                        # We'll reconstruct the new arglist: args[0], args[1], "doc", remaining...
                        new_args = []
                        for i_arg, (atext, astart, aend) in enumerate(args):
                            if i_arg == 0:
                                new_args.append(atext)
                            elif i_arg == 1:
                                new_args.append(atext)
                                # insert docstring as a new arg
                                new_args.append(f'"{escaped}"')
                            else:
                                new_args.append(atext)
                        new_arglist = ", ".join(new_args)
                        # Preserve original whitespace around parentheses by replacing the arglist region
                        new_def_text = text[start:paren_open+1] + new_arglist + text[paren_close]
    ...
    print("Insertion complete (fixed).")


def main():
    extract_docs_from_xml()
    # dump for debugging already done inside extract
