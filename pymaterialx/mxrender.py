...
import inspect, os, argparse, sys, math
...
def createWorkingDocument():
    ...
    try:
        ...
        status = '- Failed to load standard library definitions: "%s"' % err

    if libFiles:
        doc.importLibrary(stdlib)
        status = '- Loaded %s standard library definitions' % len(doc.getNodeDefs())

    return doc, stdlib, searchPath, status
...
def performRender(doc, stdlib, searchPath, inputFilename, 
    ...
    if shader:
        glslRenderer.addToRenderLog('- Generated shader for node: %s' % nodes[0].getNamePath())
...
def main():
    ...
    parser.add_argument('--searchPath', dest='searchPath', default="", help="Search Path. Default is empty")
    ...
    # Get absoluate path of opts.outputPath
    if opts.outputPath:    
        opts.outputPath = os.path.abspath(opts.outputPath)

    doc, stdlib, searchPath, status = createWorkingDocument()
    if not doc or not stdlib:
        print('Error creating working document %s' % status)
        exit(-1)
    else:
        print(status)
    ...
    try:
        ...
        print('Read in valid file "'"%s"'" for rendering.' % opts.inputFilename)
        searchPath.append(os.path.dirname(opts.inputFilename))        

    except mx.ExceptionFileMissing as err:
        print('File %s could not be loaded: "' % opts.inputFilename, err, '"')
    except mx.Exception as err:
        print('File %s fail to load properly: "' % opts.inputFilename, err, '"')

    if len(opts.searchPath) > 0:
        searchPath.append(opts.searchPath)
    ...
    renderLog = performRender(doc, stdlib, searchPath, opts.inputFilename, radianceFilePath, 
                  irradianceFilePath, opts.size, opts.size, geometryShape, mx.FilePath(opts.outputPath))
    print('\nRendering Log:\n----------------------------------------')
    print('\n'.join(renderLog))
