...
def imageLoaderTest():
    ...
    #self.image = mx_render.Image.create(256, 256, 4, mx_render.BaseType.UINT8 )
    #if self.image:
...
class GlslRenderer():
    ...
    def initializeGeometryHandler(self):
        ...
        # TODO: Currently missing gltf loader (CgltfLoader) from Python API
    ...
    def initializeLights(self, doc, enableDirectLighting, radianceIBLPath, irradianceIBLPath, enableReferenceQuality):
        ...
        # Initialize image handler if not already done so
        self.initializeImageHandler()

        self.lightHandler = mx_render.LightHandler.create()

        # Scan for lights
        if enableDirectLighting and doc:
            lights = []
        ...
        # Apply light settings for rendering.
        # Note that higher sample counts can slow down rendering signifcantly.
        ...
        # TODO: Python API missing for setRefractionTwoSided().
