...
uniform vec4 absval_vector4_in = vec4(0.000000, 0.000000, 0.000000, 0.000000);
...
void NG_extract_color4(vec4 in1, int index, out float out1)
{
    ...
    {
        N_sw_color4_out = 0.000000;
    }
    out1 = N_sw_color4_out;
}

void NG_convert_vector4_shader(vec4 in1, out surfaceshader out1)
{
    vec4 vec4_to_color4_out = vec4(in1.x, in1.y, in1.z, in1.w);
    vec3 color4_to_color3_out = vec3(vec4_to_color4_out.x, vec4_to_color4_out.y, vec4_to_color4_out.z);
    ...
    surface_out.color = 1.000000 * color4_to_color3_out;
    surface_out.transparency = 0.000000 * vec3(1.000000, 1.000000, 1.000000);
    surface_out.color *= color4_to_float_out;
    surface_out.transparency = mix(vec3(1.0), surface_out.transparency, color4_to_float_out);
    out1 = surface_out;
}

void main()
{
    vec4 absval_vector4_out = abs(absval_vector4_in);
    surfaceshader shader_absval_vector4_out_out = surfaceshader(vec3(0.0),vec3(0.0));
    NG_convert_vector4_shader(absval_vector4_out, shader_absval_vector4_out_out);
    material material_absval_vector4_out_out = shader_absval_vector4_out_out;
    float outAlpha = clamp(1.0 - dot(material_absval_vector4_out_out.transparency, vec3(0.3333)), 0.0, 1.0);
    out1 = vec4(material_absval_vector4_out_out.color, outAlpha);
