...
export function initializeNodeEditor(theRenderer) {
    ...
    var materialFilename = 'Materials/Examples/StandardSurface/gooch_shade.mtlx';
    var inputFilename = new URLSearchParams(document.location.search).get("file");
    if (inputFilename && uriExists(inputFilename)) {
        console.log('Override default file on load: ', materialFilename);
        materialFilename = inputFilename;
    }

    var geometryFilename = 'Geometry/teapot.glb'
    if (theRenderer)
    {
        viewer = theRenderer.initializeViewer(materialFilename, geometryFilename);
    ...
    function addUIHandlers() {
        ...
        function saveToStringUI() {
            ...
            let currentMaterials = viewer.getMaterial()._materials; 
            //console.log('>>> currentMaterials: ', currentMaterials);
            let renderableItems = [];
            for (let i in currentMaterials)
            {
                var matassign = currentMaterials[i];
                if (matassign) {
                    renderableItems.push(matassign.getMaterial().getNamePath())
                }
                updateRenderableItemUI(renderableItems);
                //console.log('material: ', matassign.getMaterial().getNamePath());
                //var shader = matassign.getShader(); 
        ...
        var graphtoxml = document.getElementById('graphtoxml2');
        graphtoxml.addEventListener('click', function () {
            saveToStringUI();
        });
                ...
                fetch(filePath)
                    ...
                    .then(blob => {
                        ...
                        reader.onload = function (e) {
                            ...
                            var mtlxdoc = document.getElementById('mtlxdoc').value;
                            editor.loadGraphFromString('mtlx', mtlxdoc, filePath, 80);
                        }
