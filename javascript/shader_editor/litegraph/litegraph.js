        ...
        */
        ...
        reloadNodes: function(folder_wildcard) {
            ...
            for (var i=0; i < script_files.length; i++) {
                ...
                if (
                    !src ||
                    src.slice(0, folder_wildcard.length) != folder_wildcard
                ) {
                    continue;
                }
		...
		fetchFile: function( url, type, on_complete, on_error ) {
			...
			{
				if (url.slice(0, 4) == "http" && LiteGraph.proxy) {
					url = LiteGraph.proxy + url.slice(url.indexOf(":") + 3);
				}
				return fetch(url)
...
*/
    ...
    LGraphCanvas.getFileExtension = function(url) {
        ...
        return url.slice(point + 1).toLowerCase();
    };
	...
	*/
    ...
    LGraphCanvas.prototype.drawSubgraphPanelLeft = function (subgraph, subnode, ctx) {
        var num = 0;//subnode.inputs ? subnode.inputs.length : 0;
        var w = 200;
        ...
        ctx.font = "14px Arial";
        ctx.textAlign = "left";
        ctx.fillText(subnode.title, 20, 34);
        // var pos = this.mouse;
    ...
    LGraphCanvas.prototype.drawBackCanvas = function() {
        ...
        if (this.graph) {
            ...
            // draw a grid
            /*
            var w = this.visible_area[1] - this.visible_area[0];
            var h = this.visible_area[3] - this.visible_area[2];
	...
	LGraphCanvas.prototype.drawLinkTooltip = function( ctx, link )
	{
		...
		if(text == null)
			return;
		text = text.slice(0,30); //avoid weird
            ...
            if (!low_quality) {
                ...
                if (title) {
                    ...
                    if (node.flags.collapsed) {
                        ...
                        ctx.fillText(
                            title.slice(0,20), //avoid urls too long
            ...
            switch (w.type) {
                ...
                case "text":
	                ...
	                if (show_text) {
                        ...
                        ctx.fillStyle = text_color;
                        ctx.textAlign = "right";
                        ctx.fillText(String(w.value).slice(0,30), widget_width - margin * 2, y + H * 0.7); //30 chars max
						ctx.restore();
                    }
                    break;
                default:
    ...
    LGraphCanvas.prototype.prompt = function(title, value, callback, event, multiline) {
        ...
        if(multiline)
	        dialog.innerHTML = "<span class='name'></span> <textarea  class='value'></textarea><button class='rounded'>OK</button>";
		else
        	dialog.innerHTML = "<span class='name'></span> <input  type='text' class='value'/><button class='rounded'>OK</button>";
        dialog.close = function() {
    ...
    LGraphCanvas.prototype.showSearchBox = function(event, options) {
        ...
        dialog.innerHTML = "<span class='name'>Search</span> <input  type='text' class='value rounded'/>";
    ...
    LGraphCanvas.prototype.showEditPropertyValue = function( node, property, options ) {
        ...
        if (type == "string" || type == "number" || type == "array" || type == "object") {
            input_html = "<input  type='text' class='value'/>";
        } else if ( (type == "enum" || type == "combo") && info.values) {
            input_html = "<select  type='text' class='value'>";
    ...
    LGraphCanvas.prototype.getNodeMenuOptions = function(node) {
        ...
        // TODO: Override this in application.
