...
(function(global) {
	...
	*/
    ...
    Subgraph.prototype.onDrawBackground2 = function (ctx, graphcanvas, canvas, pos) {
        if (this.flags.collapsed)
            return;
	...
	Subgraph.prototype.computeSize = function()
	{
        ...
        var width = compute_text_size(this.title);
        if (width < 200)
            width = 200;
		var num_inputs = this.inputs ? this.inputs.length : 0;
		var num_outputs = this.outputs ? this.outputs.length : 0;
		return [ width, Math.max(num_inputs,num_outputs) * LiteGraph.NODE_SLOT_HEIGHT + LiteGraph.NODE_TITLE_HEIGHT ];
	}
